...
export class RewardManager {
    ...
    try {
      ...
      if (referral.isServiceReferral) {
        await this.awardPoints(
          ...
          `Service referral reward for referring a patient to ${referral.serviceType?.toLowerCase().replace('_', ' ')}`,
          referralId
        );
      } else {
        ...
        await this.awardPoints(
          ...
          pointsToAward,
          TransactionType.REFERRAL_REWARD,
          `Referral reward for referring a ${referral.referredRole.toLowerCase()}`,
          referralId
        );
        ...
        await this.awardPoints(
          ...
          pointsToAward,
          TransactionType.REFERRAL_REWARD,
          `Reward for being referred by a ${referral.referrerRole.toLowerCase()}`,
          referralId
        );
      }
   ...
   */
  static async getUserRewardPoints(userId: string, userRole: Role) {
    try {
      let points = 0;
   ...
   */
  static async updateRewardSetting(key: string, value: number, adminId: string) {
    try {
