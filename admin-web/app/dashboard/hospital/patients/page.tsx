...
import { Loader2, Users, Search, UserPlus, XCircle, BadgeInfo } from 'lucide-react';
...
const HospitalPatientsPage = () => {
  ...
  const fetchHospitalData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const response = await hospitalsApi.getById(user.id);
  ...
  const handleAssignPatient = async () => {
    if (!user || !selectedPatientForModal || !hospitalInfo) return;
    setIsSubmitting(true);
    try {
      await hospitalsApi.assignPatient(hospitalInfo.id, selectedPatientForModal.id);
  ...
  const handleRemovePatient = async (patientId: string) => {
    if (!user || !hospitalInfo) return;
    const patientToRemove = assignedPatients.find(p => p.id === patientId);
    if (!patientToRemove) return;

    if (!confirm(`Are you sure you want to remove ${patientToRemove.name} from your hospital?`)) return;

    setIsSubmitting(true);
    try {
      await hospitalsApi.removePatient(hospitalInfo.id, patientId);
  ...
  if (!hospitalInfo) {
    return <div className="p-4 text-center text-sm md:text-base">Hospital information not found or access denied.</div>;
  }
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 space-y-4 md:space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Hospital Patients</h1>
          <p className="text-sm md:text-lg text-gray-500 mt-1">View and manage patients treated at {hospitalInfo.name}.</p>
        </div>
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          ...
          <DialogContent className="sm:max-w-[90vw] md:max-w-[600px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="text-lg md:text-xl">Assign Patient to {hospitalInfo.name}</DialogTitle>
              <DialogDescription className="text-sm">Search for a patient by name or email and assign them to your hospital.</DialogDescription>
            </DialogHeader>
      ...
      {assignedPatients.length > 0 ? (
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="text-lg md:text-xl text-gray-700">Hospital Patients ({assignedPatients.length})</CardTitle>
            <CardDescription className="text-sm text-gray-500">Patients currently treated at your hospital.</CardDescription>
          </CardHeader>
          <CardContent>
            {/* Mobile Cards View */}
                    ...
                    <TableRow key={patient.id}>
                      ...
                      <TableCell className="text-right">
                        <Button 
                          ...
                          onClick={() => handleRemovePatient(patient.id)}
                          disabled={isSubmitting}
                          title="Remove Patient from Hospital"
                        >
                          <XCircle className="h-4 w-4" />
                        </Button>
                      </TableCell>
        ...
        <Card className="shadow-lg">
          ...
          <CardContent className="text-center py-10">
            ...
            <p className="text-gray-500 text-sm md:text-base">No patients are currently treated at your hospital.</p>
